...
class findOneCoin {
  ...
  checkResult(id, heavy) {
    this.isAllow = true;
    const coinErr = this.arr[id];
    if (!this.arr.includes(coinErr) || id >= 12) {
      return false;
    }

    const newArr = this.arr.filter((item, index) => index !== id);
    const checkCoin = newArr.every((item) => item !== coinErr);
    if (checkCoin) {
      const checkHeavy = newArr.every((item) => item < coinErr);
      return checkHeavy === heavy;
    }
    return checkCoin;
  }
  // checkCoin() {
